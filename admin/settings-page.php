...
function course_certificate_admin_certificate_ui() {
	...
	if( isset($_POST['add_certificate']) ) {
		if(!wp_verify_nonce($_POST['course_nonce'], 'admin_certificate_ui')) {
		...
		} else if( empty($_POST['certificate_code']) || empty($_POST['std_name']) || empty($_POST['course_name']) || empty($_POST['course_hours']) || empty($_POST['cert_link']) || empty($_POST['award_date']) ) {
			...
			$cert_link = sanitize_text_field($_POST['cert_link']);
			$award_date = sanitize_text_field($_POST['award_date']);
			$editid = sanitize_text_field($_POST['editid']);
			$result = course_certificate_add_course_certificate($code, $name, $course, $hours, $cert_link, $award_date, $editid);
			if( $result == 1 ) {
				if( $editid != "" ) {
	...
	global $wpdb;
    $certificates = $wpdb->get_results( "SELECT * FROM smec_course_certificates");
	    ...
	    <div class="table-title">
	      <div class="row">
	        ...
	        <div class="col-sm-6">
			  ...
			  <a href="#addEmployeeModalExcel" class="btn btn-success" data-toggle="modal"><i class="material-icons">&#xE147;</i> <span>Add Bulk Certificates</span></a>
	      ...
	      <tbody>
	        <?php foreach ($certificatesNew as $value) { ?>
	    	 	<tr>
	                ...
	                <td class="cdob" ><a href="<?php echo $value->cert_link; ?>">View</a></td>
		...
		<form class="mt-40" method="POST">
	        ...
	        <div class="modal-body">
	          ...
	          <div class="form-group">
				...
				<input type="number" required class="form-control" name="course_hours">
	          </div>
	          <div class="form-group">
				<label>Register No</label>
				<input type="number" required class="form-control" name="certificate_code">
	          </div>
			  <div class="form-group">
				<label>Certificate Link</label>
				<input type="text"  required class="form-control" name="cert_link">
			  </div>
			  <div class="form-group">
	...
	<!-- Bulk Add Modal Modal HTML -->
	<div id="addEmployeeModalExcel" class="modal fade">
	  <div class="modal-dialog">
	    <div class="modal-content">
		<form class="mt-40"  method="POST" enctype="multipart/form-data">
	        ...
	        <div class="modal-body">
	          <div class="form-group">
	            ...
	            <input type="file" class="form-control" required name="file_name" accept="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet, application/vnd.ms-excel">
	          </div>
	          
	        </div>
	        <div class="modal-footer">
	          <input type="button" class="btn btn-default" data-dismiss="modal" value="Cancel">
	          <input type="submit" class="btn btn-success" value="Add" name="add_excel">
	        </div>
	      </form>
	    </div>
	  </div>
